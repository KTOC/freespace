    ...
    , m_frameGlRgba(nullptr)
{
    ...
    // format converter CUDA
    {
        ...
        result = dwImageFormatConverter_initialize(&m_yuv2rgbaCUDA, cameraImageProperties.type, context);
    ...
    // format converter NvMedia
    {
        ...
        result = dwImageFormatConverter_initialize(&m_yuv2rgbaNVM, cameraImageProperties.type, context);
        if (result != DW_SUCCESS) {
            throw std::runtime_error("Cannot create pixel format converter : yuv->rgba");
        }
    }
